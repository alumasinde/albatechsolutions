...
final class Seo
{
    ...
    public static function landingPage(array $page): array
    {
        ...
        return [
            '@context' => 'https://schema.org', '@type' => $type, 'name' => $page['title'], 'url' => $url,
            'description' => $page['meta_description'] ?: ($page['excerpt'] ?? ''),
            'isPartOf' => ['@type' => 'WebSite', 'name' => Settings::get('site_name', 'AlbaTech Solutions'), 'url' => rtrim(Config::get('app.url'), '/')],
        ];
    }

    public static function service(array $service): array
    {
        $schema = ['@context' => 'https://schema.org', '@type' => 'Service', 'name' => $service['name'], 'description' => $service['summary'] ?? strip_tags((string)($service['description'] ?? '')), 'provider' => ['@type' => 'Organization', 'name' => Settings::get('site_name', 'AlbaTech Solutions')], 'areaServed' => 'KE'];
    ...
    {
        return rtrim(Config::get('app.url'), '/') . (parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?: '/');
    }
    ...
    {
        $dedicated = Settings::get('seo_default_og_image') ?: Settings::get('site_og_image_path');
