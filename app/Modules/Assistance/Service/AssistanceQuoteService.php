...
final class AssistanceQuoteService extends BaseService
{
    ...
    public function create(int $requestId,int $adminId,array $items,string $note,?string $expiresAt): array {
        ...
        $quote = $this->quotes->findAdmin($id) ?? [];
        $quote['public_token'] = $token;
        $this->notifications->quoteSent($quote);
    ...
    public function accept(string $token): array {
        ...
        if(!empty($quote['expires_at'])&&strtotime($quote['expires_at'])<time()){ $this->quotes->updateQuote((int)$quote['id'],['status'=>'expired']); return ['success'=>false,'message'=>'This quote has expired.']; }
        $this->quotes->updateQuote((int)$quote['id'],['status'=>'accepted','accepted_at'=>date('Y-m-d H:i:s')]); $this->growth->event('quote_accepted', null, !empty($quote['service_id']) ? (int)$quote['service_id'] : null, (int)$quote['assistance_request_id'], ['quote_id'=>(int)$quote['id'],'amount'=>(float)$quote['total']]); $this->quotes->addEvent((int)$quote['id'],'accepted','customer',null,'Customer accepted the quote.'); AuditLog::record('assistance_quote.accepted','assistance_quote',(int)$quote['id']); $this->notifications->quoteAccepted($quote); return ['success'=>true];
    }
    ...
    public function verifyPayment(int $paymentId,int $adminId): array {
        ...
        if($p['status']==='verified')return ['success'=>true];
        if($p['status']!=='submitted')return ['success'=>false,'message'=>'Payment is not awaiting verification.'];
        if(abs((float)$p['amount']-(float)$p['total'])>0.001)return ['success'=>false,'message'=>'Payment amount does not match the quote.'];
        $token=bin2hex(random_bytes(24)); $this->quotes->updatePayment($paymentId,['status'=>'verified','verified_by'=>$adminId,'verified_at'=>date('Y-m-d H:i:s'),'receipt_token_hash'=>hash('sha256',$token),'receipt_token_encrypted'=>$this->encryptToken($token),'receipt_issued_at'=>date('Y-m-d H:i:s')]); $this->quotes->updateQuote((int)$p['quote_id'],['status'=>'paid','paid_at'=>date('Y-m-d H:i:s')]); $this->growth->event('payment_verified', null, !empty($p['service_id']) ? (int)$p['service_id'] : null, (int)$p['assistance_request_id'], ['payment_id'=>$paymentId,'amount'=>(float)$p['amount']]); $this->quotes->addEvent((int)$p['quote_id'],'payment_verified','admin',$adminId,'Payment verified and quote marked paid.'); AuditLog::record('assistance_payment.verified','assistance_payment',$paymentId,['quote_id'=>$p['quote_id']]); $verified=$this->quotes->findPayment($paymentId) ?? $p; $this->notifications->paymentVerified($verified, rtrim(Config::get('app.url',''),'/').'/receipt/'.rawurlencode($token)); return ['success'=>true,'receipt_token'=>$token];
    }
    ...
    {
        $key=hash('sha256',(string)Config::get('app.key',''),true); $raw=base64_decode($encrypted,true);
        if($raw===false || strlen($raw)<28) return null; $iv=substr($raw,0,12); $tag=substr($raw,12,16); $cipher=substr($raw,28);
        $plain=openssl_decrypt($cipher,'aes-256-gcm',$key,OPENSSL_RAW_DATA,$iv,$tag); return $plain===false?null:$plain;
    }
    ...
    private function encryptToken(string $token): string
    {
        ...
        if($cipher===false) throw new \RuntimeException('Unable to protect quote token.'); return base64_encode($iv.$tag.$cipher);
    }
