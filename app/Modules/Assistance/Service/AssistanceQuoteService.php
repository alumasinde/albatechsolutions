...
final class AssistanceQuoteService extends BaseService
{
    ...
    public function create(int $requestId,int $adminId,array $items,string $note,?string $expiresAt): array {
        ...
        $quote=$this->quotes->findAdmin($id)??[]; $quote['public_token']=$token; $this->notifications->quoteSent($quote);        return ['success'=>true,'id'=>$id,'number'=>$number,'token'=>$token];
    }
    ...
    public function accept(string $token,array $data=[]): array {
        ...
        $this->quotes->updateQuote((int)$quote['id'],['status'=>'accepted','accepted_at'=>date('Y-m-d H:i:s')]);        $this->quotes->addEvent((int)$quote['id'],'accepted','customer',null,'Customer accepted the quote.');
    ...
    public function submitPayment(string $token,array $data): array {
        ...
        $this->notifications->paymentSubmitted($quote,$id,$ref);return ['success'=>true,'reference'=>$ref];
    }
    ...
    public function verifyPayment(int $paymentId,int $adminId): array {
        ...
        $this->quotes->updateQuote((int)$p['quote_id'],['status'=>'paid','paid_at'=>date('Y-m-d H:i:s')]);        $this->quotes->addEvent((int)$p['quote_id'],'payment_verified','admin',$adminId,'Payment verified and quote marked paid.');AuditLog::record('assistance_payment.verified','assistance_payment',$paymentId,['quote_id'=>$p['quote_id']]);
