...
final class AuthService extends BaseService
{
    ...
    public function attemptLogin(string $email, string $password): array
    {
        ...
        if ($this->isLockedOut($user)) return ['success' => false, 'message' => 'Account temporarily locked due to repeated failed attempts. Try again later.'];
        if (empty($user['password'])) return ['success' => false, 'message' => 'This account uses Google Sign-In. Please use the "Sign in with Google" button.'];
        ...
        if (!empty($user['is_active']) === false) return ['success' => false, 'message' => 'This account has been deactivated.'];
    ...
    public function verifyTwoFactorChallenge(string $code): array
    {
        ...
        if ($userId === null) return ['success' => false, 'message' => 'No pending login to verify.'];
        $user = $this->users->find($userId);
        if (!$user) return ['success' => false, 'message' => 'No pending login to verify.'];
    ...
    private function consumeRecoveryCode(array $user, string $submitted): bool
    {
        ...
        foreach ($codes as $hash) {
            if (!$matched && is_string($hash) && password_verify($submitted, $hash)) {
                $matched = true;
                continue;
            }
            if (is_string($hash) && $hash !== '') $remaining[] = $hash;
        }

        if (!$matched) return false;

        $this->users->update((int) $user['id'], [
            'two_factor_recovery_codes' => json_encode($remaining, JSON_THROW_ON_ERROR),
        ]);
        return true;
    }
        ...
        if ($user) {
            if (!$this->isCustomerOnly((int) $user['id'])) {
                if (empty($user['email_verified_at'])) {
                    return ['success' => false, 'message' => 'Your staff account must have a verified email address before signing in with Google.'];
                }
                if (!empty($user['two_factor_enabled'])) {
                    Auth::markPendingTwoFactor((int) $user['id']);
                    AuditLog::record('login.google_verified_awaiting_2fa', 'user', (int) $user['id']);
                    return ['success' => true, 'requires_2fa' => true];
                }
            }
    ...
    private function isCustomerOnly(int $userId): bool
    {
        $stmt = \App\Core\Database::connection()->prepare(
            'SELECT COUNT(*) FROM user_roles ur INNER JOIN roles r ON r.id = ur.role_id WHERE ur.user_id = :user_id AND r.slug != :customer_slug'
        );
    ...
    public function resetPassword(string $rawToken, string $newPassword): array
    {
        ...
        if (!$tokenRow) return ['success' => false, 'message' => 'This reset link is invalid or has expired. Please request a new one.'];
        $this->users->update((int) $tokenRow['user_id'], ['password' => self::hashPassword($newPassword), 'failed_login_attempts' => 0]);
    ...
    public function verifyEmail(string $rawToken): array
    {
        ...
        if (!$tokenRow) return ['success' => false, 'message' => 'This verification link is invalid or has expired.'];
    ...
    {
        if ((int) ($user['failed_login_attempts'] ?? 0) < self::MAX_FAILED_ATTEMPTS) return false;
        if (empty($user['last_failed_login_at'])) return false;
        return time() < strtotime($user['last_failed_login_at']) + (self::LOCKOUT_MINUTES * 60);
    }
}
