...
<section class="assist-hero">
  <div class="public-container assist-hero__grid">
    <div>
      ...
      <div class="assist-hero__trust"><span><i class="fa-solid fa-user"></i> Human help</span><span><i class="fa-solid fa-shield-halved"></i> Clear process</span><span><i class="fa-brands fa-whatsapp"></i> WhatsApp friendly</span></div>
    </div>
    <aside class="assist-hero__note"><strong>Before you send anything</strong><p>Do not include passwords, PINs, OTPs, bank details or other secrets in this form. We'll tell you what information is actually needed.</p></aside>
  </div>
</section>

<section class="assist-section">
  <div class="public-container assist-layout">
    <div class="assist-form-card">
      <div class="assist-form-head"><span class="public-kicker">Get assistance</span><h2>What do you need help with?</h2><p>One request is enough. We'll contact you using your preferred method.</p></div>
      ...
      <form method="POST" action="/get-help" class="assist-form" novalidate>
        <?= csrf_field() ?>
        <div class="assist-hp" aria-hidden="true"><label>Website<input type="text" name="website" tabindex="-1" autocomplete="off"></label></div>
        <div class="assist-grid-2">
          ...
          <label>Email <span class="assist-optional">optional</span><input type="email" name="email" maxlength="190" value="<?= e($old['email'] ?? '') ?>" autocomplete="email"></label>
        ...
        <fieldset class="assist-choice-group"><legend>What are you trying to do? *</legend><div class="assist-choice-grid"><?php foreach ($categoryLabels as $value=>$label): ?><label class="assist-choice"><input type="radio" name="category" value="<?= e($value) ?>" <?= ($old['category'] ?? '') === $value ? 'checked' : '' ?>><span><?= e($label) ?></span></label><?php endforeach; ?></div></fieldset>
        <label>Specific service <span class="assist-optional">optional</span><select name="service_id" id="assist-service-select"><option value="">I'm not sure</option><?php foreach ($services as $service): ?><option value="<?= (int)$service['id'] ?>" <?= (string)($old['service_id'] ?? '') === (string)$service['id'] ? 'selected' : '' ?>><?= e($service['name']) ?></option><?php endforeach; ?></select></label>
        <?php foreach ($services as $service): $qs = json_decode((string)($service['intake_questions'] ?? '[]'), true); if (!is_array($qs) || !$qs) continue; ?>
        <div class="assist-service-questions" data-service-id="<?= (int)$service['id'] ?>" hidden>
          ...
          <?php foreach ($qs as $q): if (!is_array($q) || empty($q['key']) || empty($q['label'])) continue; $key=preg_replace('/[^a-zA-Z0-9_-]/','',(string)$q['key']); ?>
            <label><?= e($q['label']) ?> <?= !empty($q['required']) ? '*' : '<span class="assist-optional">optional</span>' ?>
        ...
        <label class="assist-consent"><input type="checkbox" name="consent" value="1" <?= ($old['consent'] ?? '') === '1' ? 'checked' : '' ?> required><span>I agree that AlbaTech may use these details to respond to this assistance request.</span></label>
